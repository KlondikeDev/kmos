...
void idt_set_gate(uint8_t num, uint64_t base) {
    ...
    idt[num].selector = 0x28;  // Kernel code segment
    idt[num].ist = 0;
    idt[num].flags = 0x8E;     // Present, Ring 0, 64-bit interrupt gate
    idt[num].reserved = 0;
}

void idt_gate_setting(){
...
void isr_handler(registers_t* regs) {
    switch(regs->int_no) {
        case 0:
            panic("DIVIDE BY ZERO", regs);
            break;
        case 1:
            panic("DEBUG EXCEPTION", regs);
            break;
        case 2:
            panic("NON-MASKABLE INTERRUPT", regs);
            break;
        case 3:
            panic("BREAKPOINT EXCEPTION", regs);
            break;
        case 4:
            panic("OVERFLOW EXCEPTION", regs);
            break;
        case 5:
            panic("BOUND RANGE EXCEEDED", regs);
            break;
        case 6:
            panic("INVALID OPCODE", regs);
            break;
        case 7:
            panic("DEVICE NOT AVAILABLE", regs);
            break;
        case 8:
            panic("DOUBLE FAULT", regs);
            break;
        case 9:
            panic("COPROCESSOR SEGMENT OVERRUN", regs);
            break;
        case 10:
        ...
        case 14:
            ...
            break;
        case 16:
            panic("FLOATING POINT EXCEPTION", regs);
            break;
        case 18:
            terminal_printf(WHITE, BLACK, "MAJOR HARDWARE FAILURE REPORTED!\n"); // Not really needed, but cool looking I guess. Might be kinda fearmongering.
            panic("MACHINE CHECK EXCEPTION", regs);
            break;
        case 129:
            if ((regs->cs & 0x3) != 0) {
                ...
                panic("MANUALLY INITIATED PANIC VIA INT 0x81", regs);
            }
            break;
        case 130:
            if ((regs->cs & 0x3) != 0) {
                ...
                panic("PHYSICAL MEMORY MANAGER FAULT", regs);
            }
            break;
        default:
            panic("UNKNOWN ERROR", regs);
            break;
    }
}
...
void idt_init() {
    ...
    kserialout("About to set IDT gates...\n");
    idt_gate_setting();
    kserialout("Gates set!\n");

    __asm__ volatile("lidt %0" : : "m"(idtp));

    kserialout("IDT loaded!\n");

    pic_remap();
    kserialout("PIC remapped!\n");

    __asm__ volatile("sti");
    kserialout("Interrupts enabled!\n");
}